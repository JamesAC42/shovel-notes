...
const createCheckoutSession = async (req, res, stripe, redis) => {
    ...
    try {
        ...
        const hadFreeTrial = await new Promise((resolve, reject) => {
            redis.sismember('shovel:free_trial_users', user.id, (err, reply) => {
                if (err) reject(err);
                resolve(reply === 1);
            });
        });
        ...
        let sessionOptions = {
            ...
            success_url: 'https://notes.ovel.sh/paymentSuccess',
            cancel_url: 'https://notes.ovel.sh/paymentFailed',
